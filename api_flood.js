...
(async function () {
    ...
    function buildForecastSeries(historyApiValues, historyDates, historyRain, forecastRainValues, forecastDates, todayApi) {
        ...
        forecastDates.forEach((date, index) => {
            ...
            forecastSeries.push({
                ...
                rfri: toFixed3((0.68 * (rain / 84.0)) + (0.32 * (nextApi / 176.44)))
            });
        });
    ...
    async function calculateDublinAPI(retries = 3) {
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                const todayRFRI = toFixed3((0.68 * (todayRain / 84.0)) + (0.32 * (todayApi / 176.44)));
                ...
                const historySeries = historyDates.map((date, index) => ({
                    ...
                    rfri: toFixed3((0.68 * (historyRain[index] / 84.0)) + (0.32 * (apiSeries[index] / 176.44))),
                    isForecast: false
                }));

                const forecastSeries = buildForecastSeries(apiSeries, historyDates, historyRain, forecastRain, forecastDates, todayApi).map(item => ({ ...item, isForecast: true }));

                state.dublin = {
                    api: Number(todayApi.toFixed(0)),
                    fi: todayRFRI,
    ...
    function createPanel() {
        ...
        if (!document.getElementById('rfri-footer')) {
            ...
            footer.style.cssText = 'position: absolute; left: 20px; bottom: 20px; color: rgba(0,0,0,0.9); font-size:12px; padding:6px 8px; border-radius:6px; z-index: 998; font-family: system-ui, sans-serif; pointer-events: none; text-align: left;';
            footer.innerHTML = '<div style="font-weight:600; font-size:13px; opacity:0.95;">NBS Digital Twin Project</div><div style="font-size:12px; opacity:0.85; margin-top:2px;">Weitao Zhang</div>';
